...
fn read_meminfo() -> io::Result<MemStats> {
    Err(io::Error::new(
        io::ErrorKind::Other,
        "Unsupported platform: blackline only supports Linux",
    ))
}

pub(crate) fn read_meminfo_path(path: &Path) -> io::Result<MemStats> {
...
// Get battery percentage and status, if present
fn get_battery_stats() -> Option<(u64, String)> {
    ...
    Some((perc, status.trim().to_string()))
}

// Get aggregated network traffic (KB received & transmitted) from /proc/net/dev
...
fn get_top_processes(n: u8) -> io::Result<Vec<(String, u64)>> {
    let output = Command::new("ps").args(["aux", "--sort=-rss"]).output()?;
...
fn display_compact(
    ...
    let mem_str = if args.delta {
        if let Some(prev) = prev_mem {
            let delta = stats.used_kb as i64 - prev;
...
mod tests {
    ...
    #[test]
    fn parses_sample_meminfo() {
        let tmp_path = "/tmp/sample_meminfo.txt";
        ...
        let stats = read_meminfo_path(Path::new(tmp_path)).expect("parse failed");
        assert_eq!(stats.total_kb, 16384256);
        assert_eq!(stats.available_kb, 8901234);
        assert_eq!(stats.used_kb, 16384256 - 8901234);
        let percent = (stats.used_kb as f64 / stats.total_kb as f64) * 100.0;
        assert!((stats.used_percent - percent).abs() < 0.01);
    }

    #[test]
    fn parses_cpu_stat() {
        let line = "cpu  1234567  23456  34567  8901234  12345  6789  4321  0";
        let raw = parse_cpu_line(line).expect("parse failed");
        assert_eq!(raw.user, 1234567);
        assert_eq!(raw.nice, 23456);
        assert_eq!(raw.system, 34567);
        assert_eq!(raw.idle, 8901234);
        assert_eq!(raw.iowait, 12345);
    }

    #[test]
    fn bar_creation() {
        let bar = create_bar(50.0, 10, true);
        assert_eq!(bar, "[#####-----]");
    }

    #[test]
    fn format_human_edge_cases() {
